...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'create') {
        ...
        $upd_stmt = $pdo->prepare("UPDATE system_accounts SET expire_date = ?, page_limit = ?, youtube_multi_api = ?, drive_multi_api = ? WHERE id = ?");
        $upd_stmt->execute([
        ...
        <tbody>
            <?php foreach ($accounts as $acc): ?>
                <tr>
                    ...
                    <td>
                        <?php if ($acc['role'] !== 'admin'): ?>
                            <button onclick="openEditModal(<?php echo $acc['id']; ?>, '<?php echo htmlspecialchars($acc['username']); ?>', '<?php echo empty($acc['expire_date']) ? '' : date('Y-m-d\TH:i', strtotime($acc['expire_date'])); ?>', <?php echo (int)$acc['page_limit']; ?>, <?php echo (int)$acc['youtube_multi_api']; ?>, <?php echo (int)$acc['drive_multi_api']; ?>)" style="background: none; border: none; color: var(--primary-color); cursor: pointer; padding: 0; margin-right: 10px; font-size: 13px; text-decoration: underline;">Sửa LH</button>
                        <?php endif; ?>
...
<div id="editLimitModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:9999; align-items:center; justify-content:center;">
    <div style="background:#fff; padding:25px; border-radius:8px; width:100%; max-width:400px; position:relative;">
        ...
        <form method="POST" action="accounts.php">
            ...
            <div class="form-group" style="margin-bottom: 25px;">
                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                    <input type="checkbox" id="e_drive_multi_api" name="drive_multi_api" value="1" style="width: 18px; height: 18px; accent-color: var(--primary-color);">
...
<script>
function openEditModal(id, username, expire, limit, youtube_multi_api, drive_multi_api) {
