...
if ($search !== '') {
    ...
    $search_where = " AND (
        ...
        OR c.id IN (
            ...
            LEFT JOIN youtube_channels yt1 ON sp.page_id = CAST(yt1.channel_id AS CHAR) AND sp.post_type = 'YouTube'
            LEFT JOIN youtube_channels yt2 ON sp.page_id = CAST(yt2.id AS CHAR) AND sp.post_type = 'YouTube'
            LEFT JOIN buffer_channels bc ON sp.page_id = CAST(bc.channel_id AS CHAR) AND sp.post_type LIKE 'Buffer%'
    ...
    if (!empty($page_camps)) {
        ...
        try {
            $users_stmt = $pdo->query("
                ...
                LEFT JOIN youtube_channels yt1 ON sub.page_id = CAST(yt1.channel_id AS CHAR) AND sub.post_type = 'YouTube'
                LEFT JOIN youtube_channels yt2 ON sub.page_id = CAST(yt2.id AS CHAR) AND sub.post_type = 'YouTube'
                LEFT JOIN buffer_channels bc ON sub.page_id = CAST(bc.channel_id AS CHAR) AND sub.post_type LIKE 'Buffer%'
