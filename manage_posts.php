    ...
    if (!empty($page_camps)) {
        ...
        try {
            $users_stmt = $pdo->query("
                ...
                LEFT JOIN users u ON p.user_id = u.id
                LEFT JOIN instagram_accounts ig2 ON CAST(sub.page_id AS BINARY) = CAST(ig2.id AS BINARY) AND sub.post_type LIKE 'Instagram%'
                LEFT JOIN youtube_channels yt1 ON CAST(sub.page_id AS BINARY) = CAST(yt1.channel_id AS BINARY) AND sub.post_type = 'YouTube'
                LEFT JOIN youtube_channels yt2 ON CAST(sub.page_id AS BINARY) = CAST(yt2.id AS BINARY) AND sub.post_type = 'YouTube'
                LEFT JOIN buffer_channels bc ON CAST(sub.page_id AS BINARY) = CAST(bc.channel_id AS BINARY) AND sub.post_type LIKE 'Buffer%'
                LEFT JOIN tiktok_accounts tt ON CAST(sub.page_id AS BINARY) = CAST(tt.id AS BINARY) AND sub.post_type = 'TikTok'
