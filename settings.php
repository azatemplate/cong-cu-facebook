...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $account_id = $_SESSION['account_id'];
    $stmt = $pdo->prepare("SELECT fb_app_id, fb_app_secret, gg_client_id, gg_client_secret, gg_refresh_token, post_delay_seconds, retry_interval_minutes, max_retries, telegram_bot_token, telegram_chat_id, tiktok_client_key, tiktok_client_secret, email, login_by_email, sales_list FROM system_accounts WHERE id = ?");
    $stmt->execute([$account_id]);
    $account = $stmt->fetch(PDO::FETCH_ASSOC);
}
...
<div style="display: flex; flex-wrap: wrap; gap: 20px; align-items: flex-start;">
    ...
    <?php if ($is_admin): ?>
    <div class="card" style="margin: 0; box-sizing: border-box;">
        <h3 style="margin-bottom: 10px;">🎵 Cấu hình TikTok Developer App (Hệ Thống)</h3>
        <p style="color: var(--text-muted); font-size: 13px; margin-bottom: 15px;">
            Nhập <b>Client key</b> và <b>Client secret</b> từ TikTok Developer Portal (Login Kit & Content Posting API). Cấu hình này do Admin quản lý và dùng chung cho tất cả người dùng trong hệ thống.
        </p>
        <form method="POST" action="settings.php">
            <?php echo csrf_field(); ?>
            <div class="form-group">
                <label>TikTok Client Key</label>
                <input type="text" name="tiktok_client_key" value="<?php echo htmlspecialchars($tiktok_client_key); ?>" placeholder="Nhập Client key..." style="width: 100%; padding: 10px; border: 1px solid var(--border-color); border-radius: 6px; box-sizing: border-box; font-family: monospace; font-size: 13px;">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" name="tiktok_scopes" value="<?php echo htmlspecialchars($tiktok_scopes); ?>" placeholder="user.info.basic,video.publish,video.upload,video.list" style="width: 100%; padding: 10px; border: 1px solid var(--border-color); border-radius: 6px; box-sizing: border-box; font-family: monospace; font-size: 13px;">
