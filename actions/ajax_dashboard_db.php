...
// 3. Followers diff from snapshot
...
if ($is_admin) {
    ...
    $stmt3 = $pdo->prepare("
        (SELECT p.*, u.name as user_name FROM pages p JOIN users u ON p.user_id = u.id WHERE u.account_id = :aid3)
        UNION
        (SELECT p.*, u.name as user_name FROM pages p JOIN page_shares ps ON p.page_id = ps.page_id JOIN users u ON p.user_id = u.id WHERE ps.shared_with_account_id = :aid4)
