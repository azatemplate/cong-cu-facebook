...
try {
    ...
    if ($exists) {
        $stmt = $pdo->prepare("UPDATE zalo_settings SET phone_request_enabled = ?, phone_request_hours = ?, phone_request_text = ?, province_request_text = ?, product_request_text = ? WHERE account_id = ?");
        $stmt->execute([$phone_request_enabled, $phone_request_hours, $phone_request_text, $province_request_text, $product_request_text, $account_id]);
    } else {
        $stmt = $pdo->prepare("INSERT INTO zalo_settings (account_id, phone_request_enabled, phone_request_hours, phone_request_text, province_request_text, product_request_text) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$account_id, $phone_request_enabled, $phone_request_hours, $phone_request_text, $province_request_text, $product_request_text]);
    }
