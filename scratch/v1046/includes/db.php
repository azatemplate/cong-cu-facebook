...
try {
    ...
    // Auto-save base_site_url once per session when accessed via Web HTTP
    if (!empty($_SERVER['HTTP_HOST']) && empty($_SESSION['base_site_url_saved'])) {
        ...
        try {
            ...
            if (session_status() === PHP_SESSION_ACTIVE) {
                $_SESSION['base_site_url_saved'] = true;
            }
        } catch (Exception $e) {}
    }
...
function ensure_db_schema_ready($pdo) {
    ...
    $flag_file = sys_get_temp_dir() . '/fb_schema_init_v15.done';
