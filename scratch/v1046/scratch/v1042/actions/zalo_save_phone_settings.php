...
$phone_request_limit = isset($_POST['phone_request_limit']) ? intval($_POST['phone_request_limit']) : 3;
...
if ($phone_request_limit < 1) $phone_request_limit = 1;
if ($phone_request_limit > 10) $phone_request_limit = 10;
...
try {
    ...
    if ($exists) {
        $stmt = $pdo->prepare("UPDATE zalo_settings SET phone_request_enabled = ?, phone_request_hours = ?, phone_request_text = ?, province_request_text = ?, product_request_text = ?, followup_request_enabled = ?, followup_request_hours = ?, followup_request_text = ?, phone_request_limit = ? WHERE account_id = ?");
        $stmt->execute([$phone_request_enabled, $phone_request_hours, $phone_request_text, $province_request_text, $product_request_text, $followup_request_enabled, $followup_request_hours, $followup_request_text, $phone_request_limit, $account_id]);
    } else {
        $stmt = $pdo->prepare("INSERT INTO zalo_settings (account_id, phone_request_enabled, phone_request_hours, phone_request_text, province_request_text, product_request_text, followup_request_enabled, followup_request_hours, followup_request_text, phone_request_limit) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$account_id, $phone_request_enabled, $phone_request_hours, $phone_request_text, $province_request_text, $product_request_text, $followup_request_enabled, $followup_request_hours, $followup_request_text, $phone_request_limit]);
    }
