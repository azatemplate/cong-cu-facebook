...
foreach ($notifs as $n) {
    if (isset($page_tokens[$n['page_id']])) {
        ...
        $res = fb_api_request($n['sender_id'], ['fields' => 'name', 'access_token' => $token]);
        if ($res['status_code'] === 200 && !empty($res['data']['name'])) {
            $name = $res['data']['name'];
            $pdo->prepare("UPDATE page_notifications SET sender_name = ? WHERE id = ?")->execute([$name, $n['id']]);
            echo "Updated id {$n['id']} with name: $name\n";
            $updated++;
        } else {
            echo "Failed for id {$n['id']}: " . json_encode($res['data']) . "\n";
        }
    }
}
