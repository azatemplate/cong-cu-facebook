...
$url    = trim($_GET['url'] ?? $_GET['video_id'] ?? '');
$count  = max(1, min(2000, intval($_GET['count'] ?? 50)));
$cursor = trim($_GET['cursor'] ?? '0');

if ($url === '') {
    http_response_code(400);
    echo json_encode(['code' => -1, 'msg' => 'Thiếu tham số url hoặc video_id']);
    exit;
}
...
if (!$data || ($data['code'] ?? 0) !== 200) {
    ...
    echo json_encode([
        'code' => -1,
        'msg' => $data['detail'] ?? ($data['msg'] ?? 'Không thể tải bình luận của video này.')
