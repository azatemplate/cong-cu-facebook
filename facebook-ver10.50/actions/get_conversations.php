...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // Check Redis Cache (2s TTL for fast polling)
    $rq = RedisQueue::getInstance();
    $cache_key = "lc_fb_convs:a{$account_id}_m{$merge_all}_p{$page_id}_f{$filter}_o{$offset}_s" . md5($search);
    if ($search === '') {
