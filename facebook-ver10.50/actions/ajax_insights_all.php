...
// Chiến lược tối ưu:
// 1. Trả JSON insights từ Redis Cache (<1ms) nếu có
// 2. Trả JSON insights về browser NGAY LẬP TỨC (không chờ followers)
// 3. Sau khi đóng HTTP connection, cập nhật followers bằng multi-curl song song
...
// ── Trả cache Redis / Session ngay nếu còn hạn (<1ms) ──────────────────────────
if (!$nocache) {
    $cached_redis = $rq->getCache($cache_key_all);
    if ($cached_redis !== false && is_array($cached_redis)) {
        header('Content-Type: application/json');
        echo json_encode(['status' => 'success', 'data' => $cached_redis]);
        exit;
    }
    
    if (isset($_SESSION[$cache_key_all]) && $_SESSION[$cache_key_all]['expires'] > time()) {
        header('Content-Type: application/json');
