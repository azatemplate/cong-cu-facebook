...
function replace_message_tags($text, $customer_name, $sales_phone) {
    $text = str_replace("\r", "", $text); // Đảm bảo chuyển \r\n thành \n cho đúng chuẩn hiển thị tin nhắn
                    ...
                    if ($remaining_seconds <= 0) {
                        ...
                        if (!empty($msg_to_send)) {
                            ...
                            if ($res['status_code'] === 200) {
                                ...
                                // Cập nhật cuộc hội thoại đệm fb_conversations để đẩy lên đầu
                                try {
                                    ...
                                    ");
                                    $stmt_conv->execute([$page_id, $c['sender_id'], $customer_name, $message_text]);
                                } catch (Exception $e) {}
                    ...
                    if ($remaining_seconds <= 0) {
                        if (empty($c['followup_requested_at'])) {
                            ...
                            if ($res['status_code'] === 200) {
                                ...
                                /*
                                // Khóa chatbot 24 giờ để tránh chatbot tự động trả lời khi khách hàng phản hồi tin CSKH (Đã tắt theo yêu cầu)
                                try {
                            ...
                            if (isset($res['status_code']) && $res['status_code'] === 200 && isset($res['data']['error']) && $res['data']['error'] === 0) {
                                ...
                                $upd_msg = $pdo->prepare("
                                    ...
                                    ON DUPLICATE KEY UPDATE 
                                        ...
                                        updated_time = CURRENT_TIMESTAMP
                                ");
                                $upd_msg->execute([$oa_id, $c['sender_id'], $customer_name, $message_text]);
...
$rq_lock->releaseLock('lock:cron:auto_request_phone');
?>
