...
<div class="page-title">Video Scheduler</div>

<div class="card">
    <h3 style="margin-bottom: 15px;">Tạo bài viết Video</h3>
    ...
    videoForm.addEventListener('submit', function(e) {
        ...
        .then(data => {
            ...
            if (data.status === 'success') {
                videoResult.className = 'alert alert-success';
                ...
                if (data.redirect) {
                    ...
                    videoResult.innerHTML += ' <a href="https://facebook.com/' + data.post_id + '" target="_blank">Xem Video</a>';
                }
                videoForm.reset();
                window.pageSelectorFilterByUser('');
            } else {
                videoResult.className = 'alert alert-danger';
                videoResult.innerHTML = data.msg;
            }
        ...
        .catch(error => {
            videoResult.style.display = 'block';
            videoResult.className = 'alert alert-danger';
            videoResult.innerHTML = 'Lỗi mạng hoặc hệ thống (Video quá lớn hoặc timeout).';
            btnSubmit.disabled = false;
            btnSubmit.textContent = 'Đăng Ngay';
        });
    });
