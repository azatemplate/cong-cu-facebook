...
$redis_active = $rq->isAvailable();
$redis_queue_len = $rq->getQueueLength('fb_publish_queue');
$redis_comment_len = $rq->getQueueLength('fb_comment_queue');
$redis_insights_len = $rq->getQueueLength('fb_insights_queue');
...
@media (max-width: 1150px) {
    .dashboard-layout {
        display: flex;
        flex-direction: column-reverse; /* Put Main Dashboard content on TOP for mobile view */
        gap: 20px;
    }
...
/* ── Mobile Phone Optimization (Screen <= 768px) ────────────────────────────── */
@media (max-width: 768px) {
    ...
    /* ── Transform Tables to Responsive Mobile Cards ───────────────────────── */
    ...
    .table-mobile-cards td[data-label="Thông tin lỗi"] {
        flex-direction: column;
        align-items: flex-start;
        text-align: left;
    }
    .table-mobile-cards td[data-label="Thông tin lỗi"]::before {
        margin-bottom: 4px;
    }
    .table-mobile-cards td[data-label="Ngưỡng kích hoạt"] {
        flex-direction: column;
        align-items: flex-start;
        gap: 4px;
    }
...
<div class="dashboard-wrapper">
    ...
    <div class="dashboard-layout">
        ...
        <aside class="sidebar-layout">
            ...
            <!-- Redis Queue Status -->
            <div class="card" style="border-top: 3px solid <?= $redis_active ? 'var(--color-success)' : 'var(--color-warning)' ?>;">
                <h3 class="card-title">
                    ...
                    Hàng đợi Redis Queue
                </h3>
                <div class="info-list">
                    <div class="info-item">
                        <span class="info-label">Trạng thái Redis:</span>
                        <span class="info-value">
                            <?php if ($redis_active): ?>
                                <span class="badge published">Hoạt động (Phúc đáp 0.001s)</span>
                            <?php else: ?>
                                <span class="badge warning">Sử dụng DB Fallback</span>
                            <?php endif; ?>
                        </span>
                    </div>
                    <div class="info-item">
                ...
                <div class="table-responsive table-mobile-cards">
                    <table style="width: 100%;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td data-label="Trạng thái" style="padding: 8px 8px;"><span class="badge <?= $s ?>"><?= $s ?></span></td>
                                <td data-label="Tổng" style="padding: 8px 8px; text-align: right; font-weight: 600;" class="mono"><?= number_format($stats[$s] ?? 0) ?></td>
                                <td data-label="Hôm nay" style="padding: 8px 8px; text-align: right; font-weight: 600;" class="mono">
                ...
                <div class="table-responsive table-mobile-cards">
                    <table style="width: 100%;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td data-label="Tài khoản" style="padding: 8px 8px; font-weight: 600; color: #a5b4fc;"><?= htmlspecialchars($acc['username']) ?></td>
                                <td data-label="Bài lỗi" style="padding: 8px 8px; text-align: right; font-weight: 700; color: var(--color-danger);" class="mono"><?= number_format($acc['cnt']) ?></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
        ...
        <main class="main-layout">
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td data-label="Loại"><span style="background: rgba(255,255,255,0.05); padding: 2px 6px; border-radius: 4px;"><?= $p['post_type'] ?></span></td>
                                <td data-label="Trạng thái"><span class="badge <?= $p['status'] ?>"><?= $p['status'] ?></span></td>
                                <td data-label="Retry" class="mono"><?= $p['retry_count'] ?? 0 ?>/<?= $p['limit_retries'] ?></td>
                                <td data-label="Giờ hẹn" class="mono"><?= $p['scheduled_time'] ?></td>
                                <td data-label="Trễ" class="text-danger font-weight-bold"><?= (int)$p['overdue_minutes'] ?> phút</td>
                                <td data-label="Thông tin lỗi" style="color:#f87171; max-width: 250px; font-size:12px;" title="<?= htmlspecialchars($p['error_msg'] ?? '') ?>">
                                    <?= htmlspecialchars($p['error_msg'] ?? '-') ?>
                                </td>
                                <td data-label="Hành động" style="text-align: right;">
                                    <a href="?delete_post=<?= $p['id'] ?>" class="btn btn-danger" style="padding: 6px 12px; font-size: 11px;" onclick="return confirm('Bạn có chắc chắn muốn xóa bài ID #<?= $p['id'] ?>?');">🗑 Xóa</a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
            ...
            <div class="card" style="border-top: 4px solid var(--color-info);">
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td data-label="Loại"><span style="background: rgba(255,255,255,0.05); padding: 2px 6px; border-radius: 4px;"><?= $p['post_type'] ?></span></td>
                                <td data-label="Giờ hẹn" class="mono"><?= $p['scheduled_time'] ?></td>
                                <td data-label="Cập nhật cuối" class="mono"><?= $p['updated_at'] ?></td>
                                <td data-label="Đã chạy" class="<?= $p['duration_min'] > 20 ? 'text-danger font-weight-bold' : 'text-warning' ?> mono">
                ...
                <div style="margin-top: 18px;">
                    <a class="btn btn-danger" style="width: 100%;" href="?force_reset=1" onclick="return confirm('Bạn có chắc chắn muốn giải phóng các bài viết đang bị treo?');">
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td data-label="ID" class="mono font-weight-bold">#<?= $p['id'] ?></td>
                                <td data-label="Tài khoản"><span style="color:#a78bfa; font-weight: 600;"><?= htmlspecialchars($p['account_name'] ?? 'System') ?></span></td>
                                <td data-label="Facebook Post ID" class="mono">
                                ...
                                <td data-label="Ngưỡng kích hoạt">
                                ...
                                <td data-label="Giờ đăng" class="mono"><?= $p['scheduled_time'] ?></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                ...
                <div class="table-responsive table-mobile-cards">
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td data-label="ID" class="mono font-weight-bold">#<?= $u['id'] ?></td>
            ...
            <div class="card">
                ...
                <div class="table-responsive table-mobile-cards" style="margin-bottom: 16px;">
                    <table>
                        <thead>
                            <tr>
                                <th>Tên Tệp Khóa</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td data-label="Tệp Khóa" class="mono font-weight-bold" style="font-size:11px; word-break: break-all;"><?= htmlspecialchars($lf['file']) ?></td>
                                <td data-label="Thời gian" class="mono"><?= $lf['age_min'] ?> phút trước</td>
                                <td data-label="Trạng thái">
                                    <?php if ($lf['locked']): ?>
                ...
                <div>
                    <a class="btn btn-secondary" style="color: var(--color-danger); border-color: rgba(239,68,68,0.2); width:100%;" href="?clear_locks=1" onclick="return confirm('Bạn có chắc chắn muốn xóa toàn bộ Lock Files?');">
