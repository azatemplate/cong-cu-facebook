...
do {
    if ($is_campaign_run) {
        // Chế độ chạy theo Campaign: 1 Worker chạy tuần tự tất cả bài trong Campaign theo đúng thời gian
        $sql = "
            SELECT sp.*, sa.max_retries AS sa_max_retries, sa.retry_interval_minutes AS sa_retry_interval, sa.post_delay_seconds AS sa_delay
            FROM scheduled_posts sp 
            LEFT JOIN system_accounts sa ON sp.account_id = sa.id 
    ...
    // CHỈ xáo trộn ngẫu nhiên nếu KHÔNG PHẢI chạy theo Campaign
    if (!$is_campaign_run) {
