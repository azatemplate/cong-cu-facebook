...
// 2. Thu thập danh sách page_id + tên trang (Cache Session 60s để tăng tốc)
$cache_key = 'my_page_ids_cache_' . $account_id;
$time_key  = 'my_page_ids_time_' . $account_id;

if (!empty($_SESSION[$cache_key]) && !empty($_SESSION[$time_key]) && (time() - $_SESSION[$time_key] < 60)) {
    $my_page_ids     = $_SESSION[$cache_key]['my_page_ids'];
    $page_names_map = $_SESSION[$cache_key]['page_names_map'];
} else {
    $page_names_map = [];
    ...
    $_SESSION[$cache_key] = [
        'my_page_ids'     => $my_page_ids,
        'page_names_map' => $page_names_map
    ];
    $_SESSION[$time_key] = time();
}
session_write_close(); // Giải phóng session lock ngay sau khi đọc/ghi cache
