...
## 8. Cấu hình Cron Dọn Dẹp Server (Server Disk Cleanup Script - 30 Phút/Lần)
Khi người dùng hoặc hệ thống chạy tự động liên tục, ổ đĩa server có thể bị phình to bởi log, thùng rác aaPanel, file tạm cURL/TikTok trong `/tmp` và file tạm Google Drive trong `/temp`.
Kịch bản Bash tối ưu dọn dẹp tổng thể (Cron: 30 phút / 1 lần, dạng Shell Script trên aaPanel):
